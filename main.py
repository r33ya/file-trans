...
UID: {user_id} 此 ID 可用于设置管理员。"""
    ...
    async def reset(self, message: AstrMessageEvent):
        ...
        if not cid:
            message.set_result(MessageEventResult().message("当前未处于对话状态。"))
            return
...
"""
    ...
    async def switch_conv(self, message: AstrMessageEvent, index: int = None):
        ...
        if index is None:
            message.set_result(MessageEventResult().message("请输入对话序号。/ls 查看对话"))
            return
        conversations = await self.context.conversation_manager.get_conversations(message.unified_msg_origin)
    ...
    async def del_conv(self, message: AstrMessageEvent):
        ...
        if provider and provider.meta().type == 'dify':
            ...
            message.set_result(MessageEventResult().message("删除当前对话成功。不再处于对话状态。"))
            return
...
""").use_t2i(False))
    ...
    async def set_variable(self, event: AstrMessageEvent, key: str, value: str):
        ...
        yield event.plain_result(f"会话 {uid} 变量 {key} 存储成功。")
            ...
            if need_active:
                ...
                try:
                    ...
                    if provider.meta().type != 'dify':
                        ...
                        if not session_curr_cid:
                            logger.error("当前未处于对话状态，无法主动回复，请确保 平台设置->会话隔离(unique_session) 未开启。")
                            return
                        ...
                        if cid is None:
                            logger.error("[Dify] 当前未处于对话状态，无法主动回复，请确保 平台设置->会话隔离(unique_session) 未开启。")
                            return
